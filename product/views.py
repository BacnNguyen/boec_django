from user.models import ReplyComment
...
class DetailView(View):
    def get(seft,request,id):
        ...
        reply_list = []
        for cm in comments:
            temp = ReplyComment.objects.filter(comment = cm)
            if len(temp)>0:
                reply_list.append(cm.id)
                reply_comment[cm.id] = temp[0]
        context['reply_list'] = reply_list
        context['reply'] = reply_comment
